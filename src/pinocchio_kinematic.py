...
class Kinematics:
    ...
    def createSolver(self):
        ...
        self.opti.minimize(
            20.0 * self.translational_cost + 0.01 * self.rotation_cost + 0.0 * self.regularization_cost + 0.005 * self.smooth_cost)
...
if __name__ == "__main__":
    ...
    arm = Kinematics("wrist_3_link")
    arm.buildFromMJCF("../model/universal_robots_ur5e/ur5e.xml")
    tf = utils.transform2mat(0.1, 0.0, 0.3, np.pi, 0, 0)
    dof, info = arm.ik(tf)
    print(f"DoF: {dof}, Info: {info}")
    print(f"FK: {arm.fk(dof)}")

    arm2 = Kinematics("wrist_3_link")
    arm2.buildFromMJCF("../model/universal_robots_ur5e/ur5e.xml")
